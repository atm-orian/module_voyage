	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if (!empty($conf->global->MAIN_GENERATE_PROPOSALS_WITH_PICTURE)) {
			...
			for ($i = 0; $i < $nblines; $i++) {
				...
				if (!empty($conf->global->PRODUCT_USE_OLD_PATH_FOR_PHOTO)) {
					$pdir[0] = pdf_azur_mention . modules ."/photos/";
					$pdir[1] = pdf_azur_mention . modules .'/';
				} else {
					$pdir[0] = get_exdir(0, 0, 0, 0, $objphoto, 'product'); // default
					$pdir[1] = pdf_azur_mention . modules ."/photos/"; // alternative
				}
		...
		if ($conf->propal->multidir_output[$conf->entity]) {
			...
			if (file_exists($dir)) {
				...
				$pdf->SetKeyWords($outputlangs->convToOutputCharset($object->ref) . " pdf_azur_mention.modules.php" .$outputlangs->transnoentities("PdfCommercialProposalTitle")." ".$outputlangs->convToOutputCharset($object->thirdparty->name));
				...
				if (!empty($conf->global->MAIN_ADD_CREATOR_IN_NOTE) && $object->user_author_id > 0) {
					...
					$creator_info = $langs->trans("CaseFollowedBy") . ' pdf_azur_mention.modules.php' .$tmpuser->getFullName($langs);
			...
			if (!empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_IFNULL) && $tvaisnull) {
				...
				foreach ($this->localtax1 as $localtax_type => $localtax_rate) {
					...
					foreach ($localtax_rate as $tvakey => $tvaval) {
						if ($tvakey != 0) {    // On affiche pas taux 0
							...
							$totalvat = $outputlangs->transcountrynoentities("TotalLT1", $mysoc->country_code) . ' pdf_azur_mention.modules.php';
							$totalvat .= pdf_azur_mention . modules;
				...
				foreach ($this->localtax2 as $localtax_type => $localtax_rate) {
					...
					foreach ($localtax_rate as $tvakey => $tvaval) {
						if ($tvakey != 0) {    // On affiche pas taux 0
							...
							$totalvat = $outputlangs->transcountrynoentities("TotalLT2", $mysoc->country_code) . ' pdf_azur_mention.modules.php';
							$totalvat .= pdf_azur_mention . modules;
							$pdf->MultiCell($col2x - $col1x, $tab2_hl, $totalvat, 0, 'L', 1);
				...
				foreach ($this->tva_array as $tvakey => $tvaval) {
					if ($tvakey != 0) {    // On affiche pas taux 0
						...
						$totalvat = ' / ' . $outputlangsbis->transcountrynoentities("TotalVAT", $mysoc->country_code);
						$totalvat .= ' ';
